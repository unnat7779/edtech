...
export default function EditTestForm({ testId }) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setFormData({
            ...
            type: data.test.type || "full-syllabus",
            chapter: data.test.chapter || "",
            class: data.test.class || "11",
            totalMarks: data.test.totalMarks || 300,
            instructions: data.test.instructions || [],
            isActive: data.test.isActive !== undefined ? data.test.isActive : true,
          })
        } else {
          setErrors({ load: "Failed to load test data" })
        }
      } catch (error) {
        setErrors({ load: "Error loading test data" })
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ submit: "Failed to update test. Please try again." })
      setButtonState("idle")
    } finally {
      setSaving(false)
    }
      ...
      case "loading":
        return (
          <div className="flex items-center justify-center">
            ...
            <span className="ml-2 sm:ml-3 animate-pulse text-sm sm:text-base">Saving Changes...</span>
          </div>
        )
      case "success":
        return (
          <div className="flex items-center justify-center">
            ...
            <span className="ml-2 sm:ml-3 text-sm sm:text-base">Changes Saved!</span>
          </div>
        )
      default:
        return (
          <>
            <Save className="h-4 w-4 sm:h-5 sm:w-5 mr-2 sm:mr-3 transition-transform group-hover:rotate-12" />
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400 text-lg">Loading test details...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-4 lg:px-8">
        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-slate-400 text-sm sm:text-base lg:text-lg max-w-2xl mx-auto px-4">
            Update test details and settings. Duration is fixed at 3 hours for all subjects.
          </p>
        </div>

        {errors.load && (
          <div className="mb-6 p-4 bg-gradient-to-r from-red-900/50 to-red-800/50 border border-red-700/50 text-red-300 rounded-lg flex items-center">
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl sm:rounded-2xl shadow-2xl border border-slate-700/50 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-teal-600 to-blue-600 p-4 sm:p-6">
            ...
            <p className="text-teal-100 text-sm sm:text-base">Update your test settings and details</p>
          </div>
          ...
          <div className="p-4 sm:p-6 lg:p-8">
            {success && (
              <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gradient-to-r from-green-900/50 to-green-800/50 border border-green-700/50 text-green-300 rounded-lg flex items-center animate-slideInDown">
                ...
                <span className="text-sm sm:text-base">Test updated successfully! Redirecting...</span>
              </div>
            )}
